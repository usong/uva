...
int main(){
	...
	while( scanf("%d %d", &n ,&m ) == 2 ){
		...
		item->pAarray = (unsigned char*)malloc( sizeof( unsigned char ) * MAX * MAX ) ; 
		item->n = n ; item->m = m;
		buf[step] = item;
		
	
		initial( item->pAarray );
		getstring( item->pAarray , n );
		for( i = 1 ; i <= n ; i++ ) { 
			for ( j = 1; j <= m ; j++ ){
				if( *(item->pAarray + ( i * MAX) + j ) != '*' ) 
					draw_mine( item->pAarray, i , j );
			}
		}			
		step++;
	}
